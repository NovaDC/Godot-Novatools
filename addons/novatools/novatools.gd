...
## The [EditorSettings] name_or_path of the setting for this system's python's executable prefix.
...
## The name_or_path of the godot editor icons theme type.
...
static func generate_version_py(to_path:String) -> Error:
	...
	ver_file.store_line('name_or_path="Godot Engine"')
...
static func get_editor_icon_named(name_or_path:String, manual_size:=Vector2i.ONE) -> Texture2D:
	assert(Engine.is_editor_hint())
	...
	assert(theme.has_icon(name_or_path, EDITOR_ICONS_THEME_TYPE))
	
	var texture := theme.get_icon(name_or_path, EDITOR_ICONS_THEME_TYPE)
...
static func callv_vcs_method(name_or_path:StringName, args:Array = []) -> Variant:
	...
	if not singleton.has_method(name_or_path) and singleton.has_method("_" + name_or_path):
		name_or_path = "_" + name_or_path
	return singleton.callv(name_or_path, args)
...
## with the name_or_path if it is active.
static func try_callv_vcs_method(name_or_path:StringName,
								 args:Array = [],
								 default:Variant = null
								) -> Variant:
	if vcs_active():
		return callv_vcs_method(name_or_path, args)
	return default
